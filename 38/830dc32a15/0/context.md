...
evaluate these suggestions: Incremental [*]
  Prevent race consuming completion signals
  Medium
  Possible issue
  Cancel operation on timeout
  Medium
  Prevent stale space query results
  Low
...
it seems we can skip all but i do liek the idea of implementing 4. Use correct sector size (Low)

  - Status: low priority on this SD-only target.
  - Current math assumes 512-byte sectors: sd_card_manager.c:1175.
  - Recommendation: optional portability hardening: query sector size from media/geometry API instead of hardcoding.
