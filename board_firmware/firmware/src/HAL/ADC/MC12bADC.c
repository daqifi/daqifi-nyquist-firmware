...
static double __attribute__((__unused__)) Convert2sComplementToSignedInt(uint32_t uVal) {
    if (uVal & 0x80000000) {
        uVal = (~uVal) + 1;
        return -(double) uVal;
    } else {
...
double MC12b_ConvertToVoltage (                                              \
    ...
    /*
        ...
        if(isDiff)
        {
            dataOut = ( 2.5 * scale * CalM * Convert2sComplementToSignedInt(sample->Value))/              \
                            (pModuleConfigMC12->Resolution/2) + runtimeConfig->CalB;  //TODO: Needs to be reviewed
        }
        else
        {
     */
