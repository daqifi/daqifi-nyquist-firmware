...
size_t Json_Encode( BoardData* state, NanopbFlagsArray* fields, uint8_t** ppBuffer )
{
    ...
    size_t startIndex = snprintf( charBuffer, JSON_ENCODER_BUFFER_SIZE, "\n\r{\n\r" );
    size_t i=0;
        ...
        switch( fields->Data[ i ] )
        {
            ...
            case DaqifiOutMessage_ip_addr_tag:
            {
                ...
                tmpLen = IpAddr_ToString( \
                        wifiSettings->configFlags & TCPIP_NETWORK_CONFIG_IPV6_ADDRESS, \
                        &wifiSettings->ipAddr, \
                        tmp, \
                        TMP_MAX_LEN );
            ...
            {
                WifiSettings* wifiSettings = &state->wifiSettings.settings.wifi;
                tmpLen = min( strlen( wifiSettings->hostName ), \
            ...
            {
                WifiSettings* wifiSettings = &state->wifiSettings.settings.wifi;
                tmpLen = min( strlen(wifiSettings->ssid), \
                        TCPIP_DNS_CLIENT_MAX_HOSTNAME_LEN);
