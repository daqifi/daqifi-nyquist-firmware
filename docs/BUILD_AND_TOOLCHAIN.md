...
| `services/SCPI/SCPIInterface.{c,h}` (`SCPI_FlushPendingDelimiter`), `services/UsbCdc/UsbCdc.c` (`SCPI_USB_Write`), `services/wifi_services/wifi_tcp_server.c` (`SCPI_TCP_Write`) | Not vendored, but the firmware-side half of the row above and listed here so the two are re-verified together. Both transports' `interface->write()` implementations call `SCPI_FlushPendingDelimiter()` first: if `pending_delimiter` is armed and the write is non-empty, it writes the `;` and clears the flag before the caller's own payload. This is the ONE choke point both `SCPI_ResultXxx()`-mediated output and a callback's direct `interface->write()` calls (`SD:LIST?`, `SYST:LOG?`, and the rest of the ~20 direct-write query callbacks) pass through, so the compound separator lands correctly for either shape without auditing each callback individually. | #1003/#1010, same defect as above — this is what makes the fix correct for direct-write callbacks instead of only ones that go through `SCPI_ResultXxx()`. | N/A — firmware-owned, not vendored; re-verify the call is still present in both `.write()` implementations after touching either transport file. |
