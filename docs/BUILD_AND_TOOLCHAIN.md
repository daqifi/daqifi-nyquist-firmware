...
| `libraries/scpi/libscpi/src/parser.c`, `src/error.c`, `src/parser_private.h` | (a) `writeDelimiter` (`parser.c:109`) now owns BOTH result separators: `,` when `output_count > 0`, `;` when `output_count == 0 && !first_output`, and clears `first_output` itself. (b) The speculative `if (!first_output && is_query) writeData(";")` that stood at the top of `processCommand` (`parser.c:203`) is DELETED — its `first_output = FALSE` flip on a successful query (`parser.c:243`) is deliberately KEPT, because several firmware query callbacks (`SD:LIST?`, `SD:GET?`, `SYST:LOG?`, …) bypass `SCPI_ResultXxx` and write straight to `interface->write`, so only that flip gets them their closing `SCPI_LINE_ENDING`. (c) New non-static `scpiParser_TerminatePendingOutput` (`parser.c:177`, declared `parser_private.h:55`), called from `SCPI_ErrorEmit` (`error.c:114`) under `err != 0`, immediately before `interface->error(...)`. (d) `writeNewLine` re-arms `first_output = TRUE` after writing (`parser.c:147`) and `SCPI_Init` sets it TRUE over the `memset` (`parser.c:296`), so the flag means exactly "something unterminated is on the wire" outside a parse too | #1003 / #1010. `SCPI_ErrorPushEx` emits synchronously, and both transports — `SCPI_USB_Error` (`services/UsbCdc/UsbCdc.c:1224`) and `SCPI_TCP_Error` (`services/wifi_services/wifi_tcp_server.c:228`) — format `**ERROR: %d, "%s"\r\n` onto the SAME `interface->write` that carries query results, while `SCPI_Parse` defers the response newline to a single `writeNewLine` after every unit. In a compound message the error text therefore landed INSIDE the previous query's open response: `*IDN?;FOO:BAR?` → `…,01-02**ERROR: -113, "Undefined header"\r\n\r\n` (no separator, plus a spurious blank line), and `*IDN?;SYST:COMM:LAN:DNS2?` → `…,01-02;**ERROR: -200, …` — a value separator introducing an error sentence, because the `;` was written before the callback that failed ever ran. Reachable from every error path, not just `SCPI_NotImplemented`: any callback returning `SCPI_RES_ERR`, and every argument-range reject in the firmware. `error.c` is the choke point because the undefined-header reject (`SCPI_Parse`) never enters `processCommand` at all. Error-queue contents and count are untouched — that is #888 | **Re-apply, all four parts.** Upstream has none of them (its `processCommand` writes the separator via `writeSemicolon` before the callback, which is the same speculative shape), so an upgrade will drop the whole patch and silently reopen both issues. Part (b)'s kept flip and part (d)'s two re-arms are each independently load-bearing — mutation-proven 2026-09-15 by removing each one alone: dropping the `writeNewLine` re-arm puts the trailing blank line back and prefixes an out-of-parse `-363` push with `\r\n`; dropping the `SCPI_Init` one prefixes a `-363` raised before the first parse; dropping the `error.c` call reverts the glued error text. The single-command case, both all-good compound cases and libscpi's own `test/test_parser.c:257/263/272` compound assertions are byte-identical before and after (A/B against `origin/main`, real libscpi linked on the host the way `tests/host/run_999_tests` does) |
