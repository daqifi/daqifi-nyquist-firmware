...
| `ScanStaleDropped` | uint32 | #557/#563 (NQ1, v3.6.2+): ticks where the shared MODULE7 scan was armed but its EOS had not fired since the prior trigger → that tick streamed **frozen** (stale) data. Counted as a genuine dropped sample (folded into the loss total at StreamingStop, unlike `EosOverruns`). Reads ~0 with the cap in place; the freeze-aware leak detector behind the NQ1 additive cap. |
...
- **`WifiTcpPartialSends` has an expected elevated band around 23–26 KB/s of wire rate (#500), and `WifiPartialBytesMissing` is NOT a loss counter (#935/#956) — it is a ring-pairing artifact.** The `WifiTcpPartialSends` counter is near-zero below ~22 KB/s and low again by ~33 KB/s. It keys on **wire byte rate, not sample rate** — 1×T1 @ 2250 Hz (24.7 KB/s) and 5×T1 @ 1250 Hz (23.6 KB/s) peak together at roughly half the sample rate and five times the channels, and both quieten at the same *byte* rates on either side. That byte-rate keying is now **partly** explained rather than merely observed (see below), and the split matters: that equal-length sends cannot expose a mis-pair is derivable from the increment site (**V** — a zero length difference contributes nothing whichever slot is read), but the claim that send lengths actually VARY mid-band and SATURATE to a constant 1400 B above it is an unmeasured distributional assumption (**I**). The `sentBytes` histogram that would settle it was deliberately not built, so the byte-rate correlation itself remains unverified. It is **not** circular-buffer-size dependent: 8 K → 64 K of `SYSTem:MEMory:WIFI:BUFfer` moves the count ~14 %, against the one-to-two orders of magnitude (32×–170× across the measured cells) between the band and its neighbouring rates. (A 4096 setting is a *different* effect — too small to sustain the rate, so throughput collapses and `WifiDroppedBytes` goes non-zero; its lower partial count is an artefact of sending far less, not an improvement.)
  ...
  **What to trust instead.** The counter that bounds real un-confirmed payload is `WifiTcpBytesSent − WifiTcpBytesConfirmed`. That difference **already includes payload still in flight** at snapshot time and must not have it added again: `wifiTcpBytesSent` is incremented when the send is issued (`wifi_tcp_server.c`, inside the ring-push critical section) and `wifiTcpBytesConfirmed` only when the completion fires (`wifi_manager.c`), so an outstanding send is counted in the first and not yet in the second (**V**). Outstanding payload is **nominally** bounded by `WIFI_TCP_MAX_IN_FLIGHT × WIFI_WBUFFER_SIZE` = 5,600 B — but treat that as design intent, **not a guaranteed ceiling**, and the reason is #956 itself: `wifi_tcp_server_TransmitBufferedData()` tests `tcpInFlight` **before** taking the mutex while `TcpServerFlush()` increments it inside a later critical section, so two producers can pass the same check at 3 and both increment, leaving five sends outstanding (**V** — the same unlocked cap that mis-pairs the ring). A difference below 5,600 B is therefore **consistent with zero loss** rather than proof of it — it is indistinguishable from sends that had not completed when the snapshot was taken. Judge permanent loss only from a **delta between two drained snapshots inside one uncontaminated epoch** — and an epoch begins at a reset taken with **nothing in flight**. "No reset inside the reading window" is **not** sufficient, which an earlier revision of this paragraph claimed: Both zero `BytesSent` and `BytesConfirmed` **without draining outstanding sends** (`SCPIInterface.c`) — the same reset asymmetry #956 names for the ring, applied to these two counters. a completion landing after such a reset adds to `Confirmed` while its `Sent` contribution was erased, and **nothing ever puts it back** — `wifiTcpBytesSent` is written in exactly three places, the `+=` at flush and the two resets — so **the offset is permanent for the rest of the session**. A later drain does not repair it, and a later reset-free window inherits it.
  ...
  Both directions are bad, and **the silent one is the likelier hazard**. `Confirmed > Sent` makes the difference negative, and since both are `uint64_t` an unsigned subtraction **wraps to ~1.8e19**, reading as catastrophic loss. But the offset can equally **cancel a real loss**: 100 B sent, `CLEar` before its completion, drain (`Sent=0, Confirmed=100`), then a clean window of 1400 issued / 1300 confirmed leaves `Sent == Confirmed == 1400` **with 100 bytes genuinely gone** — zero apparent loss, and no reset in that window. **Before #956 it was invisible in the partial counters too**, and that half no longer holds: `CLEar` used to zero the ring, so the popped `sendSize` was `0`, the `sendSize > 0` guard suppressed the partial-send flag, and only `Confirmed` moved. #956 stopped `CLEar` touching the ring at all, so an outstanding completion now pops its ORIGINAL size and a genuine short send increments the partial counters normally across a clear. The `Sent`/`Confirmed` offset described above is unaffected and still silent — it is a different pair of counters with its own reset sites. Do not read the partial counters as blind after a clear on post-#956 firmware. Two drafts of this paragraph asserted a precondition that was not the real one; this is the third, and it states what CAN be trusted rather than another list of things to avoid. **Separately, and NOT a #956 artefact:** a genuine short send is **never retried** — `TcpServerFlush` zeroes `writeBufferLength` immediately after a successful `send()` and no resend path exists, so bytes the WINC did not accept are gone regardless of ring pairing. That is a second, independent risk source; the earlier revision of this section said so, this PR's first draft dropped the sentence, and the ring-pairing explanation does not subsume it. In both #935 runs the difference was 187 B and 190 B — ~0.016–0.017 % of bytes sent and **far inside** even that nominal figure, so those runs are consistent with zero real loss. They do not support #500's original claim of exact `Sent == Confirmed` equality either, which this measurement contradicts. [Two Qodo catches on PR #957, both verified against source rather than accepted on the reviewer's word: the double-count above, and then this paragraph citing 5,600 B as a hard bound while itself documenting the race that breaks it.] **Until #956 lands, do not read a `WifiTcpPartialSends` / `WifiPartialBytesMissing` rise as evidence of lost stream bytes** — it is a diagnostic-counter defect, not a data-path one. `WifiTcpSendErrors` and `WifiDroppedBytes` remain **E**-measured 0 throughout and still only cover, respectively, a negative `sentBytes` reported by the async send *completion* in `SOCKET_MSG_SEND` (not `TcpServerFlush`'s own synchronous `send()` return code, which that function discards on success) and circular-buffer overflow — neither one was ever positioned to see this. Basis (**E**): #500's original three bench sessions 2026-08-20 (band location) + #935's two bench sessions 2026-09-09 (the counter-identity re-measurement above), all Nq1 as STA on the bench AP.

  **#956 STATUS (V, source-verified against the fix, not re-measured on hardware).** Both producer-side defects named in "Root cause" above are closed in source. (1) `TcpServerFlush` (`wifi_tcp_server.c`) now takes the authoritative `WIFI_TCP_MAX_IN_FLIGHT` check itself, inside its own `taskENTER_CRITICAL`, immediately before `send()` — the two callers' unlocked pre-checks remain only as a non-authoritative fast path. A refusal there increments a new counter, `WifiTcpInflightOverflow`, and leaves the bytes in `wCirbuf` for the next drain to retry. (2) `SYST:STR:START` and `SYST:STR:STATS:CLEar` no longer reset the ring **at all** — neither half, neither `tcpInFlight` nor `inflightSizes[]`. **An earlier revision of this paragraph said the opposite**: that both sites "now call the new `wifi_tcp_server_ResetInflightRing()`, which zeroes `tcpInFlight` together with the ring in one critical section." That was the fix's first draft and it was reversed before merge, so the claim is withdrawn rather than quietly edited — a reader who acted on it would assume the ring is clean at every stream start, which is now deliberately false. That helper no longer exists. Walk the three states, because the middle one is the trap: the ORIGINAL code reset the ring only and left `tcpInFlight` stale (the #956 defect, the missed #519 twin this section describes); resetting BOTH halves is **worse**, because accepted sends on the still-open socket can complete afterwards and completions carry no generation to distinguish them — a reset with sends outstanding admits new sends past the physical cap, and a later OLD completion pops a NEW send's slot, which is a fresh instance of the very mis-pairing #956 exists to remove. Resetting NEITHER half removes the original defect **by construction** (there is no partial reset if there is no reset) and does not create that one. The ring is live transport state owned by the send/completion pair, not a statistic, so a statistics command has no business perturbing it; the three socket-teardown sites still reset it, where it is safe because no completion can arrive for a stale slot. The adjacent comments at both SCPI sites in `SCPIInterface.c` say the same thing at the code. A second new counter, `WifiTcpOverBytesExtra`, sums the *negative* half of the pairing mismatch that the existing `<` test at `wifi_manager.c:835` discards, so `WifiTcpBytesSent − WifiTcpBytesConfirmed == WifiPartialBytesMissing − WifiTcpOverBytesExtra` is now a checkable identity rather than merely a hoped-for one (bounded by the same 5,600 B in-flight residual described above). **It holds only under conditions. An earlier revision of this sentence stated it unconditionally, which was false** (caught by PR #1024's own pre-merge audit). Two things leave a permanent *positive* difference that no pairing fault caused (**V**). The first is an **errored completion**: `SOCKET_MSG_SEND` pops the ring slot *before* it tests the sign of `sentBytes`, and on a negative value it only increments `WifiTcpSendErrors`. The send was already added to `WifiTcpBytesSent` at push, and none of `Confirmed`, `PartialBytesMissing` or `OverBytesExtra` answers it (`wifi_manager.c`). The second is a **client teardown with sends outstanding**: the three teardown sites zero `tcpInFlight` and the ring but not `WifiTcpBytesSent`, and the WINC driver discards any later completion for the closed session (`socket.c` — `shutdown()` memsets the socket entry and the `SOCKET_CMD_SEND` reply path forwards only a matching `u16SessionID`). With the ring intact (no overrun and no desyncing reset, which are the faults the identity exists to expose), the exact relation inside one clean epoch is: `(Sent − Confirmed) − (PartialBytesMissing − OverBytesExtra)` = bytes still in flight + the sizes popped by errored completions + the sizes a teardown discarded. Every term is ≥ 0. No errored completion contributes more than `WIFI_WBUFFER_SIZE` = 1,400 B, because `TcpServerFlush` clamps the length before the push. So for a **drained** snapshot from a clean epoch **with no client teardown**, the check depends on `WifiTcpSendErrors`. When it is **0**, the identity must hold exactly. When it is **E > 0**, the difference must lie in `[0, E × 1,400]`: a value outside that range is still a fault, and a value inside it is **inconclusive**, because no counter records the errored bytes themselves. Read `WifiTcpSendErrors` from the same `SYST:STR:STATS?` snapshot before judging a mismatch. Counting an errored send as 100 % missing in the firmware was considered and rejected, for two reasons. It would make the identity exact for errors only, since teardown still breaks it. And it would redefine `WifiPartialBytesMissing` partway through the #367/#935 measurement history. **What #956 did NOT touch. This paragraph supersedes exactly one claim above — the partial-counter blindness after a `CLEar`, corrected in place two paragraphs up — and nothing else:** the `Sent`/`Confirmed` reset-asymmetry offset two paragraphs up is a *different* pair of counters with its own reset sites and remains exactly as described; the never-retried genuine short send remains exactly as described. Enforcing the cap for real is also a **behaviour** change, not only an accounting one — pre-fix, a racing 5th send still went out (only its ring slot was corrupted), so the transport ceilings in `streaming.h` were fitted against a cap that leaked transient bursts above `WIFI_TCP_MAX_IN_FLIGHT`; they need at-cap re-validation on hardware before being trusted post-fix. **Nothing in this status paragraph has been measured on a device** — no bench was available to the fire that wrote it; the companion regression test is `test_956_wifi_partial_send_ring_race.py` (`daqifi-python-test-suite`), which drives 1×T1 PB @2250 Hz (the #500 band) through 5 back-to-back START/STOP cycles and asserts the identity above per cycle. A mismatch it reports is a fault only under the conditions stated above, so read that cycle's `WifiTcpSendErrors` before judging it. Until that test has actually run against real firmware, treat this paragraph as I (source-verified inference), not E.
