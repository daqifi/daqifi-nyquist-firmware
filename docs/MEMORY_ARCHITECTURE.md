...
1. **Streaming Buffer Pool** (**194,048 B** static BSS — `STATIC_POOL_SIZE` is `(194 * 1024) - 1024 - 512 - 512 - 512 - 2048`, so the "194KB" label is 4.5 KB high; `firmware/src/Util/StreamingBufferPool.c`. Was 197,632, then 197,120 after #824 paid 512 B for `streaming.c`'s `gSdHeaderBytes`, then 196,608 after #925 paid a further 512 B for `app_freertos.c`'s `gSdBusRecoveries`, and 194,560 after **#999** paid a further 2,048 B for the two per-transport `ScpiContextStorage` instances (`UsbCdc.c`'s `gUsbScpiStorage` + `wifi_tcp_server.c`'s `gTcpScpiStorage`, 546 B each, replacing one shared pair of SCPI parse-buffer/error-queue arrays with two independent ones — see `SCPIInterface.h`), and 194,048 after **#958** paid another 512 B for `SCPIStorageSD.c`'s `gBenchNameSeq`. For #824 net BSS was unchanged — 512 B trimmed for the 512 B `gSdHeaderBytes[STREAMING_SD_HEADER_MAX]`, the bytes moved. #925's 512 B funded a single 4-byte `uint32_t` (`gSdBusRecoveries`), so "unchanged" never held for it either, though its net effect was not itemised; #958 is the same case and is itemised: 512 B trimmed for one 4-byte `uint32_t`, so net BSS falls by **508 B**. **#999 trims 2,048 B for a 546 B net addition, and the table below reflects both it and #958**: it trims 2,048 B but adds 546 B net (two 546 B instances replacing one shared 546 B pair), so net BSS falls by **1,502 B**, not 2,048. An earlier revision of this line swept #999 into "every time" and the budget table credited the full 2,048 B to Free, overstating headroom by 546 B. #958 is the one payment where the link **succeeded without it** (504 bytes of stack slack on that branch base, from the map's `stack … 0x21f8 8696` against `_min_stack_size` 8192); it was paid anyway, because slack on one branch base at one moment is not slack on the merge target — and #999 landing on that target first is exactly the case the rule exists for. **On `main` at 78a0ab07 a single new `uint32_t` does not fit**: the link succeeds there and fails with `8208 bytes needed, 8200 bytes available` the moment one is added. #999's 546 B addition (before its own 2048 B trim) dropped that further to `8208 needed, 8176 available` — still failing, by 32 bytes — measured the same way, A/B in a SHA-pinned worktree (base `1faf6758d` links, head `937528950` does not). The 8208 "needed" figure is not a mystery: it is the project's linker stack-size (8192) plus stackguard (16) from `configurations.xml`, so it is fixed and only "available" moves. How much slack a given trim actually buys past the observed failing point is *not* determined by these messages and has never been measured beyond that one data point each time — the operative fact is that a modest new static is enough to break the link, so a new static needs its own payment, and this pool is where the last four came from. The 2,048 B size for #999 (larger than #824/#925's 512 B) is a judgment call, not a measured requirement: the two available link reports show BSS growth doesn't map 1:1 onto stack-region "available" (546 B added only cost 24 B of availability), so a same-size 512 B trim was not trusted to clear the 32-byte deficit with real margin; re-confirm with a real link before relying on the exact number.)
...
| Region | Bytes | Source |
|--------|------:|--------|
| Streaming Buffer Pool | 194,048 | Static BSS (`STATIC_POOL_SIZE` = 194KB - 1KB - 512B - 512B - 512B - 2KB) |
| FreeRTOS Heap | 74,000 | Static BSS |
| Coherent Pool | 126,976 | Static coherent (KSEG1) |
| USB coherent struct | ~2,000 | Static coherent |
| Other BSS/data (globals) | ~30,550 | Static BSS — includes #999's **net +546 B** (two 546 B `ScpiContextStorage` instances replacing one shared 546 B pair) and #958's 4 B `gBenchNameSeq` |
| ISR stack | 8,192 | Linker-allocated |
| **Total used** | **~436,790** | |
| **Free (linker headroom)** | **~87,010** | |

> The `~` rows are approximations, so the column does **not** add up to `Total
> used` exactly and never has — adding the rows as printed gives ~435,766. What
> this table tracks is the **delta** against the previously documented figures,
> not an exact accounting: pre-#999 it read 196,608 / ~438,800 / ~85,000, and
> #999 moves that by −1,502 B net (−2,048 pool, +546 storage), and #958 by a further −508 B net (−512 pool, +4 for `gBenchNameSeq`). Do not "fix" the
> column to sum; re-derive it from a real link if you need an exact number.
...
| Sample pool CAPACITY (slots @16ch)¹ | 1,551 | 1,100 | 1,911 | 1,053 |
...
¹ **Capacity, not the depth in use.** These are how many slots the leftover pool space holds — what `StreamingBufferPool_Partition` computes and logs as `samples=<n>x<elementBytes>`. Note the log's second number is the **element** size (72 at 16ch), not the per-sample cost — the free-list entry is counted separately, which is why the divisor below is 74 while the log shows 72. Reconciling `1600 x 72` against a `/ 74` formula otherwise looks like an inconsistency; it is not. The depth actually available is min(that, whatever the FreeRTOS sample queue could be grown to), which on the bench came out at **1100** because the queue resize was skipped for want of ~1 KB of heap (#828). That is **not** an invariant: `AInSampleList_InitializeExternal` only clamps when it tries to GROW the queue and the heap will not stretch, so a device with more free heap grows it — and once grown it stays grown for later sessions. Read the number, do not assume it.
  ...
  All four are derived from this table's own buffer values: `(194,048 - (USB + WiFi + SD + encoder circular)) / 74`, the 74 being the 16-channel element (72 B) plus its free-list entry. Only the stream-pool rows enter the sum — the three coherent-pool DMA rows come from a different 124 KB region. The formula was **confirmed against the device** at the pre-#824 pool size: with 197,632 it gives 1,600 for USB-only, and the firmware logged `samples=1600x72 (of 1600 max, 197632 pool)` on the bench 2026-08-21 alongside `SamplePoolCount=1100` for the queue-limited depth. #824 trimmed the pool by 512 B, so each column drops by ~7 slots; **#925** trimmed another 512 B (for `gSdBusRecoveries`) and **#958** a third (for `gBenchNameSeq`), ~21 slots between them; **#999** trimmed a further 2,048 B (to pay for the two per-transport `ScpiContextStorage` instances the SCPI cross-transport-corruption fix requires — see the `#define STATIC_POOL_SIZE` comment in `StreamingBufferPool.c`), ~28 slots more (2048/74). The same arithmetic reproduces the rows each side published before they were combined (196,096 → 1,579 / 1,127 / 1,939 / 1,081; 194,560 → 1,558 / 1,107 / 1,918 / 1,060).

**Whether that costs anything depends on the column, and earlier revisions of this paragraph said it never did.** The usable depth is the LESSER of the partitioned capacity and the FreeRTOS sample queue's depth (#828), so a payment is free only where the partitioned capacity stays above the queue. At 16 channels that is USB-only (1,551) and SD-only (1,911), both above the 1100-slot default. **WiFi-only now sits at exactly 1,100 — the default queue depth itself, with 8 bytes to spare**: not binding yet, but the next 512 B payment makes the pool the binding constraint there too. **USB+SD is below the queue at 1,053**, so there the pool is the binding constraint and every payment costs *usable* slots — ~7 for each 512 B trim and ~28 for #999's 2,048 B. The #1027 pre-merge audit found this for the 512 B payments, and noted the practical consequence that in that configuration an allocation of 1,082 failed after #958 where 1,088 had succeeded before it. A revision of this paragraph on `main` then called #999's payment "effectively nil cost" by the margin argument, which in USB+SD it was not. The USB-only column of the twice-recomputed figures **was re-confirmed on the bench** (2026-09-08, 16ch PB USB-only on serial `7E2898F46200E8A7`): `SYST:MEM:FREE?` reported `SamplePoolPartitioned=1586`, `SamplePoolCount=1100`, `SamplePoolClampedSlots=486`, `SampleElementBytes=72` — the 1,586 predicted at the time exactly, and the ~500-slot queue clamp of #828 intact. That bench reading predates both #958 and #999: the table above now predicts 1,551 where the device measured 1,586, and the 35-slot difference is those two payments (~7 + ~28). All four columns of the current table — **bench-confirmed 2026-09-13, crc 4C22BC41** (16ch PB, serial `7E2898F46200E8A7`, one auto-balanced `SYST:STReam:BENCHmark 1` run per column, `SYST:MEM:FREE?` read after `SYST:STReam:STOP` each time, cross-checked against both the per-transport buffer-size getters and the firmware's own `Pool partition:` log line): USB-only 1,551, WiFi-only 1,100, SD-only 1,911, USB+SD 1,053 — every predicted figure landed exactly, including the two columns (WiFi-only, USB+SD) that were arithmetic-only at every pool size this repo had shipped before this run. That run's `SamplePoolClampedSlots` read 0 in all four columns rather than the ~486 the 2026-09-08 USB-only reading showed at the earlier pool size — an artifact of that boot's FreeRTOS sample queue having already grown past every column's need from earlier activity (the queue never shrinks back), not a change to the capacity figures above, which come from `StreamingBufferPool_Partition`'s own carve independent of queue state. Earlier figures still in circulation (~1,618 / ~1,178 / ~1,921 / ~1,086) were computed with the old, wrong 512-byte SD-circular value and are wrong twice over.
