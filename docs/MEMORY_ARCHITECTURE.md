...
1. **Streaming Buffer Pool** (**194,048 B** static BSS — `STATIC_POOL_SIZE` is `(194 * 1024) - 1024 - 512 - 512 - 2048 - 512`, so the "194KB" label is 4.5 KB high; `firmware/src/Util/StreamingBufferPool.c`. Was 197,632, then 197,120 after #824 paid 512 B for `streaming.c`'s `gSdHeaderBytes`, then 196,608 after #925 paid a further 512 B for `app_freertos.c`'s `gSdBusRecoveries`, then 194,560 after **#999** paid a further 2,048 B for the two per-transport `ScpiContextStorage` instances (`UsbCdc.c`'s `gUsbScpiStorage` + `wifi_tcp_server.c`'s `gTcpScpiStorage`, 546 B each, replacing one shared pair of SCPI parse-buffer/error-queue arrays with two independent ones — see `SCPIInterface.h`), and now **194,048** after **#956** paid a further 512 B for two new `uint32_t` fields (`wifiTcpOverBytesExtra`, `wifiTcpInflightOverflow`) added to the `client` member of the static `gTcpServerContext` (`wifi_manager.c`) — see the `#define STATIC_POOL_SIZE` comment in `StreamingBufferPool.c`, whose own history records that this is the fifth trim overall and the third `-512U` token, not the fourth of either as an earlier revision of that comment (written before #999 landed on main ahead of it) miscounted. For #824 and #925 net BSS was unchanged — 512 B trimmed, 512 B added, the bytes moved. **#999 and #956 are both exceptions and the table below reflects both**: #999 trims 2,048 B but adds 546 B net (two 546 B instances replacing one shared 546 B pair), so net BSS falls by **1,502 B**, not 2,048; #956 trims 512 B but adds only 8 B net (two `uint32_t`), so net BSS falls by a further **504 B**. An earlier revision of this line swept #999 into "every time" and the budget table credited the full 2,048 B to Free, overstating headroom by 546 B — the same mistake is flagged here in advance for #956's 8 B. **On `main` at 78a0ab07 a single new `uint32_t` does not fit**: the link succeeds there and fails with `8208 bytes needed, 8200 bytes available` the moment one is added. #999's 546 B addition (before its own 2048 B trim) dropped that further to `8208 needed, 8176 available` — still failing, by 32 bytes — measured the same way, A/B in a SHA-pinned worktree (base `1faf6758d` links, head `937528950` does not). The 8208 "needed" figure is not a mystery: it is the project's linker stack-size (8192) plus stackguard (16) from `configurations.xml`, so it is fixed and only "available" moves. How much slack a given trim actually buys past the observed failing point is *not* determined by these messages and has never been measured beyond that one data point each time — the operative fact is that a modest new static is enough to break the link, so a new static needs its own payment, and this pool is where the last four came from. #956's own 512 B size was chosen by following the #824/#925 precedent directly (its commit message says so), not from a fresh link-failure measurement the way #999's 2,048 B was — a rebuild + link at this exact head has not been run by the fire that merged #956 onto #999 (no build sandbox available); re-confirm with a real link before relying on the exact number. The 2,048 B size for #999 (larger than #824/#925's 512 B) is a judgment call, not a measured requirement: the two available link reports show BSS growth doesn't map 1:1 onto stack-region "available" (546 B added only cost 24 B of availability), so a same-size 512 B trim was not trusted to clear the 32-byte deficit with real margin; re-confirm with a real link before relying on the exact number.)
...
| Region | Bytes | Source |
|--------|------:|--------|
| Streaming Buffer Pool | 194,048 | Static BSS (`STATIC_POOL_SIZE` = 194KB - 1KB - 512B - 512B - 2KB - 512B) |
| FreeRTOS Heap | 74,000 | Static BSS |
| Coherent Pool | 126,976 | Static coherent (KSEG1) |
| USB coherent struct | ~2,000 | Static coherent |
| Other BSS/data (globals) | ~30,554 | Static BSS — includes #999's **net +546 B** (two 546 B `ScpiContextStorage` instances replacing one shared 546 B pair) and #956's **net +8 B** (`wifiTcpOverBytesExtra` + `wifiTcpInflightOverflow`, two `uint32_t` added to `gTcpServerContext`'s `client` member) |
| ISR stack | 8,192 | Linker-allocated |
| **Total used** | **~436,794** | |
| **Free (linker headroom)** | **~87,006** | |

> The `~` rows are approximations, so the column does **not** add up to `Total
> used` exactly and never has — adding the rows as printed gives ~435,770. What
> this table tracks is the **delta** against the previously documented figures,
> not an exact accounting: pre-#999 it read 196,608 / ~438,800 / ~85,000, #999
> moved that by −1,502 B net (−2,048 pool, +546 storage), and #956 moves it by
> a further −504 B net (−512 pool, +8 storage). Do not "fix" the column to sum;
> re-derive it from a real link if you need an exact number.
...
¹ **Capacity, not the depth in use.** These are how many slots the leftover pool space holds — what `StreamingBufferPool_Partition` computes and logs as `samples=<n>x<elementBytes>`. Note the log's second number is the **element** size (72 at 16ch), not the per-sample cost — the free-list entry is counted separately, which is why the divisor below is 74 while the log shows 72. Reconciling `1600 x 72` against a `/ 74` formula otherwise looks like an inconsistency; it is not. The depth actually available is min(that, whatever the FreeRTOS sample queue could be grown to), which on the bench came out at **1100** because the queue resize was skipped for want of ~1 KB of heap (#828). That is **not** an invariant: `AInSampleList_InitializeExternal` only clamps when it tries to GROW the queue and the heap will not stretch, so a device with more free heap grows it — and once grown it stays grown for later sessions. Read the number, do not assume it.
  ...
  All four are derived from this table's own buffer values: `(194,048 - (USB + WiFi + SD + encoder circular)) / 74`, the 74 being the 16-channel element (72 B) plus its free-list entry. Only the stream-pool rows enter the sum — the three coherent-pool DMA rows come from a different 124 KB region. The formula was **confirmed against the device** at the pre-#824 pool size: with 197,632 it gives 1,600 for USB-only, and the firmware logged `samples=1600x72 (of 1600 max, 197632 pool)` on the bench 2026-08-21 alongside `SamplePoolCount=1100` for the queue-limited depth. #824 trimmed the pool by 512 B, so each column drops by ~7 slots — off a *partitioned* capacity that already exceeds the usable depth by ~500, which is why the change costs nothing in practice **for the columns that stay above the queue clamp**. **#925** then trimmed another 512 B (to pay for `gSdBusRecoveries`), dropping each column by a further ~7 slots for the same reason and, for those same columns, at the same nil cost. The USB-only column of the twice-recomputed figures **was re-confirmed on the bench** (2026-09-08, 16ch PB USB-only on serial `7E2898F46200E8A7`): `SYST:MEM:FREE?` reported `SamplePoolPartitioned=1586`, `SamplePoolCount=1100`, `SamplePoolClampedSlots=486`, `SampleElementBytes=72` — the predicted 1,586 exactly, and the ~500-slot queue clamp of #828 intact. The other three columns remain arithmetic. Earlier figures still in circulation (~1,618 / ~1,178 / ~1,921 / ~1,086) were computed with the old, wrong 512-byte SD-circular value and are wrong twice over. **#999** then trimmed a further 2,048 B (to pay for the two per-transport `ScpiContextStorage` instances the SCPI cross-transport-corruption fix requires — see the `#define STATIC_POOL_SIZE` comment in `StreamingBufferPool.c`), dropping each column by ~28 slots (2048/74) for the same reason — **but note USB+SD's capacity had already fallen to 1,060 by this point, which is BELOW the 1,100 queue clamp**, so "nil cost" stops applying to that column starting at #824 (it was 1,095 at #824's 197,120, already under 1,100): USB+SD is capacity-limited, not queue-limited, and every byte trimmed from the pool since #824 has cost that column real usable depth. This is a pre-existing inaccuracy in this file predating #956 and is out of this PR's scope to fix; recorded separately. The recomputed #999 figures (USB-only 1,558, WiFi-only 1,107, SD-only 1,918, USB+SD 1,060) were arithmetic, not a fresh bench confirmation. **#956** then trimmed a further 512 B (to pay for `wifi_manager.c`'s `wifiTcpOverBytesExtra`/`wifiTcpInflightOverflow`, two `uint32_t` — see the `#define STATIC_POOL_SIZE` comment), dropping each column by a further ~7 slots (512/74) to the current 1,551 / 1,100 / 1,911 / 1,053. Read per column, not uniformly: USB-only and SD-only stay comfortably above the 1,100 queue clamp, so the trim is nil-cost for them exactly as #824/#925 were. **WiFi-only now lands EXACTLY on 1,100** — the same value as the queue clamp — so it is no longer comfortably queue-limited; the next trim of any size would make WiFi-only capacity-limited too. **USB+SD drops from 1,060 to 1,053, a real loss of 7 usable slots (~0.7%)**, because that column has been capacity-limited (not queue-limited) since #824, as described above — this is the one column where #956's trim is NOT nil-cost. None of the four post-#956 figures have been re-measured on hardware at this pool size; re-confirm `SYST:MEM:FREE?`'s `SamplePoolPartitioned` against 1,551 (USB-only) and `SamplePoolCount` against 1,053 (USB+SD, the capacity-limited column) once a device is flashed with this size, rather than trusting the formula alone.
