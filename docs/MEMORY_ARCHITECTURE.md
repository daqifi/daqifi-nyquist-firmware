...
1. **Streaming Buffer Pool** (**196,096 B** static BSS — `STATIC_POOL_SIZE` is `(194 * 1024) - 1024 - 512 - 512 - 512`, so the "194KB" label is 2.5 KB high; `firmware/src/Util/StreamingBufferPool.c`. Was 197,632, then 197,120 after #824 paid 512 B for `streaming.c`'s `gSdHeaderBytes`, 196,608 after **#925** paid a further 512 B for `app_freertos.c`'s `gSdBusRecoveries`, and 196,096 after **#958** paid another 512 B for `SCPIStorageSD.c`'s `gBenchNameSeq`. Every time net BSS was unchanged — the bytes moved. #958 is the one payment where the link **succeeded without it** (504 bytes of stack slack on that branch base, from the map's `stack … 0x21f8 8696` against `_min_stack_size` 8192); it was paid anyway, because slack on one branch base at one moment is not slack on the merge target. **On `main` at 78a0ab07 a single new `uint32_t` does not fit**: the link succeeds there and fails with `8208 bytes needed, 8200 bytes available` the moment one is added. How much `main` had spare is *not* determined by that message and has never been measured — the operative fact is that one scalar was enough to break it, so a new static needs its own payment, and this pool is where the last two came from.)
...
| Region | Bytes | Source |
|--------|------:|--------|
| Streaming Buffer Pool | 196,096 | Static BSS (`STATIC_POOL_SIZE` = 194KB - 1KB - 512B - 512B - 512B) |
...
| Sample pool CAPACITY (slots @16ch)¹ | 1,579 | 1,127 | 1,939 | 1,081 |
...
¹ **Capacity, not the depth in use.** These are how many slots the leftover pool space holds — what `StreamingBufferPool_Partition` computes and logs as `samples=<n>x<elementBytes>`. Note the log's second number is the **element** size (72 at 16ch), not the per-sample cost — the free-list entry is counted separately, which is why the divisor below is 74 while the log shows 72. Reconciling `1600 x 72` against a `/ 74` formula otherwise looks like an inconsistency; it is not. The depth actually available is min(that, whatever the FreeRTOS sample queue could be grown to), which on the bench came out at **1100** because the queue resize was skipped for want of ~1 KB of heap (#828). That is **not** an invariant: `AInSampleList_InitializeExternal` only clamps when it tries to GROW the queue and the heap will not stretch, so a device with more free heap grows it — and once grown it stays grown for later sessions. Read the number, do not assume it.
  ...
  All four are derived from this table's own buffer values: `(196,096 - (USB + WiFi + SD + encoder circular)) / 74`, the 74 being the 16-channel element (72 B) plus its free-list entry. Only the stream-pool rows enter the sum — the three coherent-pool DMA rows come from a different 124 KB region. The formula was **confirmed against the device** at the pre-#824 pool size: with 197,632 it gives 1,600 for USB-only, and the firmware logged `samples=1600x72 (of 1600 max, 197632 pool)` on the bench 2026-08-21 alongside `SamplePoolCount=1100` for the queue-limited depth. #824 trimmed the pool by 512 B, so each column drops by ~7 slots; **#925** trimmed another 512 B (for `gSdBusRecoveries`) and **#958** a third (for `gBenchNameSeq`), ~21 slots between them.

**Whether that costs anything depends on the column, and an earlier revision of this paragraph said it never did.** The usable depth is the LESSER of the partitioned capacity and the FreeRTOS sample queue's depth (#828), so a payment is free only where the partitioned capacity stays above the queue. At 16 channels that is USB-only (1,579), WiFi-only (1,127) and SD-only (1,939), all above the 1100-slot default. **USB+SD is the exception: 1,081, below 1100**, so there the pool is the binding constraint and all three payments cost 7 *usable* slots each. Found by the #1027 pre-merge audit, which also noted the practical consequence: in that configuration an allocation of 1,082 now fails where 1,088 succeeded. The USB-only column of the twice-recomputed figures **was re-confirmed on the bench** (2026-09-08, 16ch PB USB-only on serial `7E2898F46200E8A7`): `SYST:MEM:FREE?` reported `SamplePoolPartitioned=1586`, `SamplePoolCount=1100`, `SamplePoolClampedSlots=486`, `SampleElementBytes=72` — the 1,586 predicted at the time exactly, and the ~500-slot queue clamp of #828 intact. That bench reading predates #958's payment, so the table above now predicts 1,579 where the device measured 1,586; the 7-slot difference IS the payment and has not itself been re-measured. The other three columns remain arithmetic. Earlier figures still in circulation (~1,618 / ~1,178 / ~1,921 / ~1,086) were computed with the old, wrong 512-byte SD-circular value and are wrong twice over.
