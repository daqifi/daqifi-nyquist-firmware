...
1. **Streaming Buffer Pool** (**194,048 B** static BSS — `STATIC_POOL_SIZE` is `(194 * 1024) - 1024 - 512 - 512 - 2048 - 512`, so the "194KB" label is well over 4 KB high; `firmware/src/Util/StreamingBufferPool.c`. Was 197,632, then 197,120 after #824 paid 512 B for `streaming.c`'s `gSdHeaderBytes`, then 196,608 after #925 paid a further 512 B for `app_freertos.c`'s `gSdBusRecoveries`, 194,560 after **#999** paid a further 2,048 B for the two per-transport `ScpiContextStorage` instances (`UsbCdc.c`'s `gUsbScpiStorage` + `wifi_tcp_server.c`'s `gTcpScpiStorage`, 546 B each, replacing one shared pair of SCPI parse-buffer/error-queue arrays with two independent ones — see `SCPIInterface.h`), and 194,048 after **#914** paid a further 512 B for `sd_card_manager.c`'s two-slot `gSdAsyncError[]` (16 B: a `{int32_t code; uint32_t generation;}` pair per SD reply target, USB and WiFi-TCP, used to hand a GET open-failure's deferred SCPI error to the owning transport). For #824 and #925 net BSS was unchanged — 512 B trimmed, 512 B added, the bytes moved. **#999 is the exception and the table below reflects it**: it trims 2,048 B but adds 546 B net (two 546 B instances replacing one shared 546 B pair), so net BSS falls by **1,502 B**, not 2,048. An earlier revision of this line swept #999 into "every time" and the budget table credited the full 2,048 B to Free, overstating headroom by 546 B. **#914** is back to the #824/#925 shape: 512 B trimmed to pay for 16 B actually added, at the same established quantum rather than a byte-exact amount (net BSS falls by only 16 B, but the trim removes 512 B of pool capacity — those are two different numbers and only the pool-capacity one feeds the table below). **On `main` at 78a0ab07 a single new `uint32_t` does not fit**: the link succeeds there and fails with `8208 bytes needed, 8200 bytes available` the moment one is added. #999's 546 B addition (before its own 2048 B trim) dropped that further to `8208 needed, 8176 available` — still failing, by 32 bytes — measured the same way, A/B in a SHA-pinned worktree (base `1faf6758d` links, head `937528950` does not). The 8208 "needed" figure is not a mystery: it is the project's linker stack-size (8192) plus stackguard (16) from `configurations.xml`, so it is fixed and only "available" moves. How much slack a given trim actually buys past the observed failing point is *not* determined by these messages and has never been measured beyond that one data point each time — the operative fact is that a modest new static is enough to break the link, so a new static needs its own payment, and this pool is where the last four came from. The 2,048 B size for #999 (larger than #824/#925/#914's 512 B) is a judgment call, not a measured requirement: the two available link reports show BSS growth doesn't map 1:1 onto stack-region "available" (546 B added only cost 24 B of availability), so a same-size 512 B trim was not trusted to clear the 32-byte deficit with real margin; re-confirm with a real link before relying on the exact number.)
...
> #999 moves that by −1,502 B net (−2,048 pool, +546 storage). Do not "fix" the
