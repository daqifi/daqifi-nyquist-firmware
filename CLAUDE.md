...
- **Scan-rate bound (#541 D-C)**: retriggering the scan while in progress is documented-undefined (FRM §22.3.2) — this was #539, where EOS stopped firing above ~4.6 kHz and scanned data froze. The cap equation now includes `f_scan_max = 1/(N_active × (SAMC+2+14) × TAD7)` with all terms read live (TAD7 = 100 ns at boot clocks; silicon-verified ±2%). N_active scales with the session: 1×T2 OBDiag=0 → ~86 kHz; 16ch OBDiag=1 → ~4.8 kHz. OBDiag=1 visibly lowers `CONF:CAP` — honest physics, monitoring rides the same scan
