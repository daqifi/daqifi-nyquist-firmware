...
- **`volatile` also applies to "set-once-then-shared" pointers**, not just frequently-written variables. Pattern: a `static T *gp = NULL;` set in `Init()` (before the scheduler) and then dereferenced from BOTH SCPI tasks and priority-1 ADC ISRs. At -O3 GCC may hoist/merge address loads or cache cross-context reads of the data because the pointer "never changes" and the data has no volatile qualifier.
  ...
  4. **Don't apply pointer-only volatile speculatively.** Microchip's published best-practice list (Harmony Sync/Async drivers tech brief DS90003269A, etc.) prescribes critical sections + mutexes for cross-context shared data — NOT `volatile` for set-once pointers. Add the qualifier only when there's a concrete -O3 miscompile that goes away with it.
  5. **Don't add OR remove `T * volatile` on set-once shared pointers speculatively.** Empirically verified by codegen A/B + bench A/B (XC32 v4.60 + -O3 + no LTO, 2026-05-10, see `docs/SET_ONCE_POINTER_AUDIT.md`): the qualifier IS observable in codegen — adds redundant `lw v0,-32xxx(gp)` reloads and inhibits loop strength reduction. For most sites in this codebase, every consumer path crosses opaque function-call boundaries (FreeRTOS API, Harmony PLIB, BoardData_Get/Set) so the volatile reloads are correct-but-redundant — *don't add* the qualifier where it isn't already present. **But also don't remove existing qualifiers** without bench validation: the bench A/B for `streaming.c::buffer` showed that stripping its existing volatile produced 1 encoder failure / 589k samples while `main` had 0 / 723k under identical PAT 3 / 5 kHz deterministic test, meaning at least one execution path doesn't get the function-call-barrier protection the codegen analysis assumed. The original #354 ch15 regression (the source of the speculation that motivated #421) was actually fixed by a hardware TRGSRC register configuration (`96e7c840`), not by volatile semantics. Re-audit if LTO is enabled, if a hot-loop function call is replaced with `__attribute__((always_inline))`, or if the XC32/GCC version changes.
