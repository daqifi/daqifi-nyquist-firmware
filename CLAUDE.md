...
- **`WifiTcpPartialSends` has an expected elevated band around 23–26 KB/s of wire rate (#500) — but see the ⚠️ below: "expected" is established, "harmless" is NOT.** The counter is near-zero below ~22 KB/s and low again by ~33 KB/s. It keys on **wire byte rate, not sample rate** — 1×T1 @ 2250 Hz (24.7 KB/s) and 5×T1 @ 1250 Hz (23.6 KB/s) peak together at roughly half the sample rate and five times the channels, and both quieten at the same *byte* rates on either side. That is what rules **out** back-pressure (rates above the band push more bytes and produce far fewer partial sends — congestion cannot make a local maximum with quiet on both sides) and points, **I** not confirmed, at something keyed to bytes-per-send. The average shortfall is **11–19 B against the 1400 B send cap** (`WIFI_WBUFFER_SIZE` = `SOCKET_BUFFER_MAX_LENGTH`, `wifi_tcp_server.h`), i.e. ~1 % of a send, and it barely moves between the peak and its neighbours while the *frequency* changes. It is **not** circular-buffer-size dependent: 8 K → 64 K of `SYSTem:MEMory:WIFI:BUFfer` moves the count ~14 %, against the one-to-two orders of magnitude (32×–170× across the measured cells) between the band and its neighbouring rates. (A 4096 setting is a *different* effect — too small to sustain the rate, so throughput collapses and `WifiDroppedBytes` goes non-zero; its lower partial count is an artefact of sending far less, not an improvement.) ⚠️ **Do NOT read this band as proven harmless.** That half of #500 is contradicted by the counters' own arithmetic and is open as **#935**. `WifiTcpSendErrors` and `WifiDroppedBytes` are **E**-measured 0 throughout, but they cover only negative `send()` returns and circular-buffer overflow respectively — neither can see this shortfall. The shortfall is **not** re-queued: `TcpServerFlush` zeroes `writeBufferLength` immediately after a successful `send()` (`wifi_tcp_server.c:136`) and no retry path exists, so `WifiPartialBytesMissing` counts bytes the WINC did not accept and the firmware did not resend. From the three single increment sites, `BytesSent − BytesConfirmed ≡ WifiPartialBytesMissing` whenever `SendErrors == 0` (**V** — `wifi_tcp_server.c:133`, `wifi_manager.c:834`, `:839`); #500's report of "`Sent == Confirmed` exactly" *alongside* 7,726 missing bytes cannot satisfy that identity, so one of those two readings is wrong. Until #935 re-measures both from one response, treat a `WifiTcpPartialSends` rise as **expected in this band but not yet proven lossless** — and do not cite #500 for "nothing is lost". Both shipped WiFi endurance ceilings sit near this band, so it **will** be met. The `sentBytes` histogram that would explain the ~1 % shortfall was deliberately **not** built. Basis (**E**): three bench sessions 2026-08-20, 45 s cells, Nq1 as STA on the bench AP — the raw counts are the measurement, per-hour figures are extrapolation, and whether ~24 KB/s is universal or bench-specific is untested on a second AP.
...
1. **Streaming Buffer Pool** (**196,608 B** static BSS — `STATIC_POOL_SIZE` is `(194 * 1024) - 1024 - 512 - 512`, so the "194KB" label is 2 KB high; `firmware/src/Util/StreamingBufferPool.c`. Was 197,632, then 197,120 after #824 paid 512 B for `streaming.c`'s `gSdHeaderBytes`, and 196,608 after **#925** paid a further 512 B for `app_freertos.c`'s `gSdBusRecoveries`. Both times net BSS was unchanged — the bytes moved. **On `main` at 78a0ab07 a single new `uint32_t` does not fit**: the link succeeds there and fails with `8208 bytes needed, 8200 bytes available` the moment one is added. How much `main` had spare is *not* determined by that message and has never been measured — the operative fact is that one scalar was enough to break it, so a new static needs its own payment, and this pool is where the last two came from.)
...
| Region | Bytes | Source |
|--------|------:|--------|
| Streaming Buffer Pool | 196,608 | Static BSS (`STATIC_POOL_SIZE` = 194KB - 1KB - 512B - 512B) |
...
| Sample pool CAPACITY (slots @16ch)¹ | 1,586 | 1,134 | 1,946 | 1,088 |
