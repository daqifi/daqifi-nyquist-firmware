...
| Lane nq-b (added 2026-09-08) | Nyquist on **COM8** + **PICkit 5** `020026703RYN079002` | Three differences from nq-a, all in the registry: `*IDN?` serial reads **0** (unprovisioned — identity is the COM/hub-port only, and `0` is deliberately not a guard token); firmware predates `CONF:CAP:JSON?` so crc32 is unreadable until reflashed; pairing **UNPROVEN**. |
