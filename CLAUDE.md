...
- **Scan-rate bound (#541 D-C)**: retriggering the scan while in progress is documented-undefined (FRM §22.3.2) — this was #539, where EOS stopped firing above ~4.6 kHz and scanned data froze. Worse: with a small dynamic scan list, sustained mid-scan retriggering **kills the USB peripheral outright** (silicon-observed at n=7 @ 11,750 Hz; device off the bus until PICkit reset). The cap equation therefore bounds the tick period to the scan's true busy time plus margin: `f_scan_max = 1/(1.1 × (N_active × (SAMC+2+14) × TAD7 + 6 µs))` — the 6 µs is a per-scan fixed overhead pinned by two silicon anchors (n=7 wedge threshold 11,500/11,750; n=19 EOS timing 216 µs + 4,500 Hz clean), all other terms read live (TAD7 = 100 ns at boot clocks). N_active scales with the session: 1×T2 OBDiag=0 → ~52 kHz (never binds); 1×T1 OBDiag=1 → 10,425; 16ch OBDiag=1 → ~4.2 kHz. OBDiag=1 visibly lowers `CONF:CAP` — honest physics, monitoring rides the same scan
