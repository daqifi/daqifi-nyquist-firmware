...
- **`volatile` also applies to "set-once-then-shared" pointers**, not just frequently-written variables. Pattern: a `static T *gp = NULL;` set in `Init()` (before the scheduler) and then dereferenced from BOTH SCPI tasks and priority-1 ADC ISRs. At -O3 GCC may hoist/merge address loads or cache cross-context reads of the data because the pointer "never changes" and the data has no volatile qualifier.
  ...
  4. **Don't add OR remove `T * volatile` on set-once shared pointers speculatively** — neither direction. The default for new code is "no qualifier"; the default for existing-and-working code is "leave it alone." Microchip's published best-practice list (Harmony Sync/Async drivers tech brief DS90003269A, etc.) prescribes critical sections + mutexes for cross-context shared data — NOT `volatile` for set-once pointers. So *don't add* the qualifier where it isn't already present unless you have a concrete -O3 miscompile that goes away with it. Conversely, *don't remove* an existing qualifier even when codegen analysis says the reload is "redundant by inspection" — bench A/B for `streaming.c::buffer` (2026-05-10) showed stripping its volatile produced 1 encoder failure / 589k samples while main had 0 / 723k under identical PAT 3 / 5 kHz deterministic test, meaning the function-call-barrier theory has at least one execution path it doesn't cover. The qualifier IS observable in codegen — adds redundant pointer reloads and inhibits loop strength reduction — but for most sites in this codebase the opaque function calls in consumer paths (FreeRTOS API, Harmony PLIB, BoardData_Get/Set) already force reload at `-O3` + no LTO + this XC32 v4.60 build, so adding volatile is just dead overhead without a concrete miscompile to motivate it. The original #354 ch15 regression (the source of the speculation that motivated #421) was actually fixed by a hardware TRGSRC register configuration (`96e7c840`), not by volatile semantics. Full audit and re-audit triggers (LTO, `__attribute__((always_inline))` in a hot loop, XC32 upgrade) in `docs/SET_ONCE_POINTER_AUDIT.md`.
