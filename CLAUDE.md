...
1. **Streaming Buffer Pool** (**196,608 B** static BSS — `STATIC_POOL_SIZE` is `(194 * 1024) - 1024 - 512 - 512`, so the "194KB" label is 2 KB high; `firmware/src/Util/StreamingBufferPool.c`. Was 197,632, then 197,120 after #824 paid 512 B for `streaming.c`'s `gSdHeaderBytes`, and 196,608 after **#925** paid a further 512 B for `app_freertos.c`'s `gSdBusRecoveries`. Both times net BSS was unchanged — the bytes moved. **`main` at 78a0ab07 linked with only 8 bytes to spare** — adding one `uint32_t` failed the link with `8208 bytes needed, 8200 bytes available` — so a new static does not fit without a payment, and this pool is where the last two came from. (#925's 512 B buys back roughly 504 for whatever is next; do not read that as room to skip the payment.))
