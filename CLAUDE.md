...
- **`volatile` also applies to "set-once-then-shared" pointers**, not just frequently-written variables. The pattern that bit us in the #354 ch15 regression: a `static T *gp = NULL;` set in `Init()` (before the scheduler) and then dereferenced from BOTH SCPI tasks and priority-1 ADC ISRs. At -O3 GCC was free to hoist/merge address loads and cache cross-context reads of the data because the pointer "never changes" and the data has no volatile qualifier.
  ...
  PR #420 used the pointer-only form on `gpBoardConfig` / `gpBoardRuntimeConfig` / `gpBoardData` in `firmware/src/HAL/ADC.c` because the data-volatile form cascades into every internal ADC API. Qodo correctly flagged this trade-off in PR #420 review — the comment now documents both forms and the rationale for choosing the partial fix here. Issue #421 tracks the wider audit / data-volatile propagation as follow-up work.
