...
| #38 | System Bus | B2/B3 | **Flash wait states at SYSCLK >184 MHz with ECC need 3 wait states** (not 2). <2% CPU impact with cache. | **Safe.** At 252 MHz (#487) we run `PRECONbits.PFMWS = 5` (conservative bring-up) and `ECCCON = 3` in `initialization.c`. **#586 investigation:** the datasheet+errata minimum with ECC at 252 MHz is **4** (200 MHz needs base-2/ECC-3; +1 cadence step +1 ECC → 4). `5` carries one wait state of headroom; **`3` would be too few with ECC** (no room for the ECC read-latency cycle). Tune-down 5→4 recommended but held doc-only pending a DS60001320H table cross-check + at-speed bench soak — see `docs/PFMWS_252MHZ_2026-07.md`. |
