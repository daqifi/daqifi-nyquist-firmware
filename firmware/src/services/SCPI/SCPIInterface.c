...
static void SCPI_PerformStreamingStop(void) {
     ...
     * counter reset.
     *
     * The MEASURED reason is ownership, not the race. This body was the ONLY
     * caller of either reset, so a stop was responsible for the NEXT session's
     * header -- and any path that begins a session without coming through here
     * inherited whatever the last one left behind. SYST:STR:THRoughput and
     * SYST:STR:WIFI:FINd? are exactly those paths (they poke IsEnabled and call
     * Streaming_UpdateState directly), and on 3.7.2 a session following a
     * THRoughput run emitted NO header at all. That is what the companion test
     * asserts, and it needs no race.
     *
     * The ticket's own hazard -- a START arming between Streaming_UpdateState()
     * above and this line, so the reset lands mid-stream -- is closed by the
     * move as well, but it could NOT be reproduced on hardware: both blocking
     * calls in this window interlock the racing START (recorded on #870).
     *
    ...
    /* Clear STATus:OPERation -- but only if nothing is streaming, which is what
     * SCPI_ClearStreamingOperBits tests. This was an unconditional
     * SCPI_ClearOperBits; the header comment above records why the argument for
     * keeping it that way does not hold (#870), rather than repeating it here. */
    SCPI_ClearStreamingOperBits(pRunTimeStreamConfig);
...
static bool PrepareStreamingBuffers(uint32_t poolCount, size_t sampleElemSize) {
     ...
     * partition, and the start has to be refused -- the ticket's acceptance
     * line asks for the safe state, which is no streaming session. Every
     * caller already reads `false` as "do not start" and unwinds its own arm,
     * so this one test refuses all four entry points (START, SYST:MEM:AUTO,
     * SYST:STR:THRoughput and the WiFi rate finder) rather than each growing
     * a copy.
     *
     ...
     * half of #950 fixes, because the re-carve above has already reassigned
     * that address range. The call is the same one the success path makes;
     * with these arguments it takes its own bail-out, which is where the
     * invalidation lives, so there is one implementation rather than two kept
     * in step. */
...
static const scpi_command_t scpi_commands[] = {
    ...
    {.pattern = "DIO:CLOCk:CONFig", .callback = SCPI_DioClockConfig,},
    {.pattern = "DIO:CLOCk:ENAble", .callback = SCPI_DioClockEnable,},
    {.pattern = "DIO:CLOCk?", .callback = SCPI_DioClockGet,},
    {.pattern = "DIO:EVENt:ENAble", .callback = SCPI_DioEventEnable,},
    {.pattern = "DIO:EVENt:ENAble?", .callback = SCPI_DioEventEnableGet,},
    {.pattern = "DIO:EVENt:COUNt?", .callback = SCPI_DioEventCount,},
    {.pattern = "DIO:EVENt:NEXT?", .callback = SCPI_DioEventNext,},
    {.pattern = "DIO:COUNter:ENAble", .callback = SCPI_DioCounterEnable,},
    {.pattern = "DIO:COUNter:ENAble?", .callback = SCPI_DioCounterEnableGet,},
    {.pattern = "DIO:COUNter?", .callback = SCPI_DioCounterGet,},
    {.pattern = "DIO:COUNter:CLEar", .callback = SCPI_DioCounterClear,},
    {.pattern = "DIO:MEASure:FREQuency?", .callback = SCPI_DioMeasFrequency,},
    {.pattern = "DIO:MEASure:PERiod?", .callback = SCPI_DioMeasPeriod,},
    {.pattern = "DIO:MEASure:PWIDth?", .callback = SCPI_DioMeasPulseWidth,},
    {.pattern = "DIO:MEASure:DUTY?", .callback = SCPI_DioMeasDuty,},
    ...
    {.pattern = "SYSTem:COMMunicate:UART:ENAble?", .callback = SCPI_UartEnableGet,},
    {.pattern = "SYSTem:COMMunicate:UART:INVert", .callback = SCPI_UartInvertSet,},
    {.pattern = "SYSTem:COMMunicate:UART:INVert?", .callback = SCPI_UartInvertGet,},
    {.pattern = "SYSTem:COMMunicate:UART:WRITe", .callback = SCPI_UartWrite,},
    {.pattern = "SYSTem:COMMunicate:UART:READ?", .callback = SCPI_UartRead,},
    {.pattern = "SYSTem:COMMunicate:UART:COUNt?", .callback = SCPI_UartCount,},
    {.pattern = "SYSTem:COMMunicate:I2C:ENAble", .callback = SCPI_I2cEnableSet,},
    {.pattern = "SYSTem:COMMunicate:I2C:ENAble?", .callback = SCPI_I2cEnableGet,},
    {.pattern = "SYSTem:COMMunicate:I2C:SEGment", .callback = SCPI_I2cSegmentSet,},
    {.pattern = "SYSTem:COMMunicate:I2C:SEGment?", .callback = SCPI_I2cSegmentGet,},
    {.pattern = "SYSTem:COMMunicate:I2C:FREQuency", .callback = SCPI_I2cFreqSet,},
    {.pattern = "SYSTem:COMMunicate:I2C:FREQuency?", .callback = SCPI_I2cFreqGet,},
    {.pattern = "SYSTem:COMMunicate:I2C:SCAN?", .callback = SCPI_I2cScan,},
    {.pattern = "SYSTem:COMMunicate:I2C:TRANsfer?", .callback = SCPI_I2cTransfer,},
    // ADC
...
/* #914: the SD manager's deferred error code is a plain int (sd_card_manager.h
 * has no SCPI dependency); pin it against the real libscpi value so the two
...
void SCPI_DrainDeferredSdError(scpi_t *context) {
    ...
    const sd_card_manager_reply_target_t target =
            wifi_tcp_server_ContextIsTcp(context) ? SD_CARD_REPLY_WIFI_TCP
                                                  : SD_CARD_REPLY_USB;
    const int32_t err = sd_card_manager_TakeAsyncError(target);
    if (err != 0) {
