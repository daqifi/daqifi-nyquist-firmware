...
scpi_result_t SCPI_StorageSDListDir(scpi_t * context){
    ...
    if (fileLen > 0) {
        // Security fix - use sizeof for buffer validation
        ...
        strcpy(pSdCardRuntimeConfig->directory, "/mnt/Daqifi");
    }

    // Use sd_card_manager's async LIST_DIRECTORY mode instead of sync directory reading
    // This allows the sd_card_manager task to handle directory enumeration without
    // blocking the SCPI handler. Results will be sent via sd_card_manager_DataReadyCB
