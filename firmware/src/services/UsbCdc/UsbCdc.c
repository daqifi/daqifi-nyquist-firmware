...
static bool UsbCdc_FinalizeRead(UsbCdcData_t* client) {
    if (client->readBufferLength > 0) {
        if (client->transparentModeActive == 0) {
            ...
            return true;
        } else if (UsbCdc_ReadCmpltCB(client->readBuffer, client->readBufferLength) == true) {
            client->readBufferLength = 0;
            return true;
        }
    }
...
void UsbCdc_Initialize() {
    ...
    gRunTimeUsbSttings.transparentModeActive = true;
