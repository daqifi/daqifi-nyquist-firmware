...
#include "services/serial_bridge/serial_bridge.h"
...
typedef enum {
    ...
    WIFIAPI_CONSUMED_EVENT_FW_UPDATE_INIT,
...
struct stateMachineInst {
    ...
    bool fwUpdateProcesRunning;
    SERIAL_BRIDGE_DECODER_STATE serialBridgeDecoderState;
};
...
static WifiApi_eventStatus_t MainState(stateMachineInst_t * const pInstance, uint16_t event) {
    ...
    switch (event) {
        case WIFIAPI_CONSUMED_EVENT_ENTRY:
            ...
            pInstance->fwUpdateProcesRunning=false;
        ...
        case WIFIAPI_CONSUMED_EVENT_INIT:
            returnStatus = WIFIAPI_EVENT_STATUS_HANDLED;
            SendEvent(WIFIAPI_CONSUMED_EVENT_FW_UPDATE_INIT);
            if (WDRV_WINC_Status(sysObj.drvWifiWinc) == SYS_STATUS_UNINITIALIZED) {
            ...
            break;
        case WIFIAPI_CONSUMED_EVENT_FW_UPDATE_INIT:
        {
            if (pInstance->fwUpdateProcesRunning == false) {
                SerialBridge_Init(&pInstance->serialBridgeDecoderState);
                pInstance->fwUpdateProcesRunning = true;
            }
            break;
        }
...
void WifiApi_ProcessState() {
    ...
    if(gStateMachineData.fwUpdateProcesRunning==true){
        SerialBridge_Process(&gStateMachineData.serialBridgeDecoderState);
    }
